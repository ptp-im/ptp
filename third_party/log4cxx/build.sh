...
if [ $SYSTEM ==  "mac" ];then
  ./configure --prefix=/usr/local --with-apr=/usr/local/opt/apr/bin/apr-1-config --with-apr-util=/usr/local/opt/apr-util/bin/apu-1-config --with-charset=utf-8 --with-logchar=utf-8
  ...
  ./configure --prefix=/usr/local --with-apr=/usr --with-apr-util=/usr --with-charset=utf-8 --with-logchar=utf-8
fi
