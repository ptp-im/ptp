...
TEST(test_Group, GroupUnreadMsgAction) {
    ...
    ImPdu pdu;
    pdu.SetPBMsg(&msg,CID_GroupUnreadMsgReq,0);
    pMsgSrvConn->HandlePdu(&pdu);
    auto pPdu = pMsgSrvConn->ReadTestPdu();
    ASSERT_EQ(pPdu->GetCommandId(),CID_GroupUnreadMsgRes);
    PTP::Group::GroupUnreadMsgRes msg_rsp;
    auto res = msg_rsp.ParseFromArray(pPdu->GetBodyData(), (int)pPdu->GetBodyLength());
    ASSERT_EQ(res,true);
    //auto unread_list = msg_rsp.unread_list();
    //DEBUG_I("unread_list: %s",unread_list);
            
    //auto unread_cnt = msg_rsp.unread_cnt();
    //DEBUG_I("unread_cnt: %s",unread_cnt);
