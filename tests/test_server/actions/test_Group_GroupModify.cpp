...
TEST(test_Group, GroupModifyAction) {
    ...
    ImPdu pdu;
    pdu.SetPBMsg(&msg,CID_GroupModifyReq,0);
    pMsgSrvConn->HandlePdu(&pdu);
    auto pPdu = pMsgSrvConn->ReadTestPdu();
    ASSERT_EQ(pPdu->GetCommandId(),CID_GroupModifyRes);
    PTP::Group::GroupModifyRes msg_rsp;
    auto res = msg_rsp.ParseFromArray(pPdu->GetBodyData(), (int)pPdu->GetBodyLength());
    ASSERT_EQ(res,true);
    //auto group_modify_action = msg_rsp.group_modify_action();
    //DEBUG_I("group_modify_action: %s",group_modify_action);
    ...
    //auto notify_members = msg_rsp.notify_members();
    //DEBUG_I("notify_members: %s",notify_members);
