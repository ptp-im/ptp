...
void AccountManager::handlePdu(NativeByteBuffer * buff,uint32_t bufSize,uint32_t instanceNum){
    buff->position(0);
    ...
    if(bufSize >= pduLen){
        ...
        DEBUG_D("onRead cid=%d,pduLen:%d,encrypt:%d",pPdu->GetCommandId(),pduLen,pPdu->GetReversed());
        ...
        if(pPdu->GetCommandId() == 262){//auth captcha
            ...
            if(pPdu->GetCommandId() == 1793){//heart beat
                ...
                buffer->writeInt32(GetAccountId());
            }
            else{
...
int AccountManager::_SendPdu(CImPdu* pPdu){
    ...
    if(m_connection != nullptr){
        int SOCKET_MAX_SEND_BUF_SIZE1 = 3 * 1024;

        int requestBodyLen = (int)pPdu->GetLength();
        sentLength = requestBodyLen;
        uint32_t offset = 0;
        uint32_t remain = requestBodyLen;
        uint32_t err = 0;
        while (remain > 0) {
            uint32_t send_size = remain;
            if (send_size > SOCKET_MAX_SEND_BUF_SIZE1) {
                send_size = SOCKET_MAX_SEND_BUF_SIZE1;
            }
            NativeByteBuffer *buffer = BuffersStorage::getInstance().getFreeBuffer(send_size);
            buffer->writeBytes(pPdu->GetBuffer()+offset,send_size);
            m_connection->sendData(buffer);
            offset += send_size;
            remain -= send_size;
            Logger::d("Send: %d / %d,offset: %d\n",offset,requestBodyLen,remain);
            if(remain <=0){
                break;
            }
        }
