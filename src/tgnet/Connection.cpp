...
void Connection::connect() {
    if (currentDatacenter->instanceNum != 0) {
        return;
    }
    ...
    if (AccountManager::getCurrentAccount().isConnected() || connectionState == TcpConnectionStageConnected || connectionState == TcpConnectionStageConnecting) {
        return;
    }
...
void Connection::sendData(NativeByteBuffer *buffer) {
    if (buffer == nullptr) {
        return;
    }
    buffer->rewind();
...
//        } else {
//            waitForReconnectTimer = false;
//            if (connectionType == ConnectionTypeGenericMedia|| connectionType == ConnectionTypeGeneric && (datacenterId == ConnectionsManager::getInstance(currentDatacenter->instanceNum).currentDatacenterId || datacenterId == ConnectionsManager::getInstance(currentDatacenter->instanceNum).movingToDatacenterId)) {
