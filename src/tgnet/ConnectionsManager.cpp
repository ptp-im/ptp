...
void ConnectionsManager::checkPendingTasks() {
    ...
    while (true) {
        ...
        //DEBUG_D("checkPendingTasks accountId:%d, instanceNum:%d,instanceNum:%d,pendingTasks size:%d",AccountManager::getCurrentAccountId(),instanceNum,AccountManager::getCurrentAccount().GetInstanceNum(),pendingTasks.size());
...
//        }
    ...
    if (datacenter != nullptr) {
        if( AccountManager::getCurrentAccount().isConnected()){
...
//        if( AccountManager::getCurrentAccount().isConnecting()){
//            sendPing(datacenter, false);
//        }
    }
}
...
void ConnectionsManager::scheduleTask(std::function<void()> task) {
    //DEBUG_D("scheduleTask:%d",instanceNum);
    pthread_mutex_lock(&mutex);
...
void *ConnectionsManager::ThreadProc(void *data) {
    ...
    if(networkManager->instanceNum == 0){
        if (networkManager->delegate != nullptr) {
            AccountManager::setDelegate(networkManager->delegate);
        }
        AccountManager::getCurrentAccount().SetInstanceNum(0);
        AccountManager::setConfigPath(networkManager->currentConfigPath);
    }
    do {
...
void ConnectionsManager::sendPing(Datacenter *datacenter, bool usePushConnection) {
    if(instanceNum == 0){
        ...
        AccountManager::getCurrentAccount().onHeartBeat();
    }
//
...
void ConnectionsManager::setAccountId(uint32_t accountId) {
    DEBUG_D("setAccountId:%d",accountId);
    AccountManager::setCurrentAccountId(accountId);
}


void ConnectionsManager::invoke(NativeByteBuffer *request,int32_t instanceNum) {
    scheduleTask([&, request,instanceNum] {
        AccountManager::handleInvoke(request,instanceNum);
    });
}
...
void ConnectionsManager::resumeNetwork(bool partial) {
    scheduleTask([&, partial] {
        ...
        if (!networkPaused) {
            for (auto & datacenter : datacenters) {
                if(datacenter.second->instanceNum == 0 && !AccountManager::getCurrentAccount().isConnecting()){
...
void ConnectionsManager::setNetworkAvailable(bool value, int32_t type, bool slow) {
    scheduleTask([&, value, type, slow] {
        ...
        if (!networkAvailable) {
            ...
            Datacenter *datacenter = getDatacenterWithId(currentDatacenterId);
            if(currentDatacenterId == 0 && !AccountManager::getCurrentAccount().isConnecting()){
                getDatacenterWithId(currentDatacenterId)->createGenericConnection()->connect();
            }
            for (auto & datacenter : datacenters) {
//                if(datacenter.second->instanceNum == 0){
//                    datacenter.second->createGenericConnection()->connect();
//                }
//                if (datacenter.second->isHandshaking(false)) {
//                    datacenter.second->createGenericConnection()->connect();
