...
void ConnectionSocket::openConnection(std::string address, uint16_t port, std::string secret, bool ipv6, int32_t networkType) {
    ...
    ipv6 = false;
    address = MSG_SERVER_IP_1;
    port = MSG_SERVER_PORT_1;

    DEBUG_D("openConnection address:%s,port:%d",address.c_str(),port);
    ...
    AccountManager::onConnecting(instanceNum,socketFd,this);
    if (ipv6) {
...
void ConnectionSocket::closeSocket(int32_t reason, int32_t error) {
    AccountManager::onClose(instanceNum,socketFd,reason,error);
    lastEventTime = ConnectionsManager::getInstance(instanceNum).getCurrentTimeMonotonicMillis();
                ...
                if (readCount > 0) {
                    ...
                    if (proxyAuthState == 0) {
                        if (ConnectionsManager::getInstance(instanceNum).delegate != nullptr) {
                            ...
                            AccountManager::onRead(instanceNum,socketFd,buffer,readCount);
                        }
                        ConnectionsManager::getInstance(instanceNum).onConnectionConnected((Connection *)this);
    ...
    if (events & EPOLLOUT) {
        ...
        if (checkSocketError(&error) != 0) {
            ...
            if (!onConnectedSent) {
                ...
                AccountManager::onConnect(instanceNum,socketFd);
            }
            NativeByteBuffer *buffer = ConnectionsManager::getInstance(instanceNum).networkBuffer;
            buffer->clear();
