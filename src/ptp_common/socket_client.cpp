...
//        if (err != 0){
//            Logger::e("An error has occurred while getting address. %s.", gai_strerror(err));
//            break;
//        }
//        sock = socket(result->ai_family, result->ai_socktype, 0);
//        if (sock < 0){
//            Logger::e("A socket error has occurred");
//            break;
//        }
//        err = (int)sendto(sock, data_sent, sizeof(data_sent), 0, result->ai_addr, result->ai_addrlen);
//        if (err < 0){
//            Logger::e("failed to send request to server");
//            break;
//        }
...
int socketSend(const char* ip,uint16_t port,unsigned char *requestBody,uint32_t requestBodyLen,unsigned char *responseBody,uint32_t &responseBodyLen){
    ...
    while (true){
        ...
        {
            Logger::e("Failed to create Socket");
            break;
        }
        ...
        if ((inet_pton(AF_INET, ip, &server.sin_addr)) <= 0){
            Logger::e("Unable to convert ip address to binary form");
            break;
        }
...
//            Logger::e("An error has occurred fetching the ip of the domain");
//            break;
//        }
        ...
        //Logger::d("requestBodyLen:%d\n",requestBodyLen);
        uint32_t requestLen = requestBodyLen + headerLen;
        //Logger::d("headerLen:%d\n",headerLen);
        //Logger::d("requestLen:%d\n",requestLen);
...
//    if (!ctx)
//    {
//        fLogger::d(stderr, "Error creating SSL context\n");
...
//
//    /* name = "<hostname>:<port>" */
//    sLogger::d(name, "%s:%s", hostname, "https");
//
//    BIO_set_conn_hostname(cbio, name);
//
//    if (BIO_do_connect(cbio) <= 0)
//    {
//        fLogger::d(stderr, "Error attempting to connect\n");
//        ERR_print_errors_fp(stderr);
//        BIO_free_all(cbio);
//        SSL_CTX_free(ctx);
//        exit(1);
