...
void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    //Logger::d("msg=%d, handle=%d ", msg, handle);
	...
	case NETLIB_MSG_CLOSE:
		...
		break;
	default:
		Logger::e("!!!imconn_callback error msg: %d ", msg);
		break;
	}
...
CImConn::~CImConn()
{
	//Logger::d("CImConn::~CImConn, handle=%d ", m_handle);
}
...
//	++g_send_pkt_cnt;
	...
	if (remain > 0)
	{
		...
		m_busy = true;
		Logger::d("send busy, remain=%d ", m_out_buf.GetWriteOffset());
	}
    else
    {
        OnWriteCompelete();
...
//			++g_recv_pkt_cnt;
            ...
            Logger::e("!!!catch exception, sid=%u, cid=%u, err_code=%u, err_msg=%s, close the connection ",
