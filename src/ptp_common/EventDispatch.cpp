...
CEventDispatch::CEventDispatch()
{
	...
	if (m_kqfd == -1)
	{
Logger::d("kqueue failed");
	}
#else
	m_epfd = epoll_create(1024);
	if (m_epfd == -1)
	{
Logger::d("epoll_create failed");
	}
#endif
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			Logger::d("select failed, error code: %d", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			Logger::d("select return read count=%d", read_set.fd_count);
		...
		{
			Logger::d("select return write count=%d", write_set.fd_count);
		...
		{
			Logger::d("select return exception count=%d", excep_set.fd_count);
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
			...
			{
				//Logger::d("OnRead, socket=%d", ev_fd);
				pSocket->OnRead();
			}

			if (events[i].filter == EVFILT_WRITE)
			{
				//Logger::d("OnWrite, socket=%d", ev_fd);
				pSocket->OnWrite();
			}
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		Logger::d("epoll_ctl() failed, errno=%d", errno);
	}
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		for (int i = 0; i < nfds; i++)
		{
            ...
            {
                Logger::d("On Peer Close, socket=%d", ev_fd);
                pSocket->OnClose();
            }
            #endif
            // Commit End
			...
			{
				//Logger::d("OnClose, socket=%d", ev_fd);
				pSocket->OnClose();
			}
