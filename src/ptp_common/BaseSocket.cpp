#include "ptp_common/BaseSocket.h"
#include "ptp_common/EventDispatch.h"
...
{
	//Logger::d("CBaseSocket::CBaseSocket\n");
...
CBaseSocket::~CBaseSocket()
{
//	Logger::d("CBaseSocket::~CBaseSocket, socket=%d\n", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	{
		Logger::e("socket failed, err_code=%d\n", _GetErrorCode());
		return NETLIB_ERROR;
	}
	...
	if (ret == SOCKET_ERROR)
	{
        ...
        Logger::e("bind failed, err_code=%d", _GetErrorCode());
	...
	if (ret == SOCKET_ERROR)
	{
		Logger::e("listen failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_ERROR;
	}
	...
	Logger::d("CBaseSocket::Listen on %s:%d", server_ip, port);
...
{
	//Logger::d("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
	...
	if (m_socket == INVALID_SOCKET)
	{
       Logger::d("socket failed, err_code=%d", _GetErrorCode());
	...
	{
       Logger::d("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
#endif
			ret = 0;
Logger::d("socket send block fd=%d", m_socket);
		}
		else
		{
//           Logger::d("!!!send failed, error code: %d", err_code);
		}
	}
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	if (ret == SOCKET_ERROR) {
		Logger::d("set SO_SNDBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &size, &len);
	Logger::d("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	if (ret == SOCKET_ERROR) {
		Logger::d("set SO_RCVBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, &size, &len);
	Logger::d("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       Logger::d("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       Logger::d("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       Logger::d("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
           Logger::d("gethostbyname failed, ip=%s", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		Logger::d("AcceptNewSocket, socket=%d from %s:%d", fd, ip_str, port);
