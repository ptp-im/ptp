...
bytes_data bytes_data::from_chars(const std::vector<char>& data) {
    return ptp_toolbox::data::to_bytes(data);
}

bytes_data bytes_data::from_chars(const char* data, size_t len) {
    return ptp_toolbox::data::to_bytes(data, len);
}
...
std::ostream& ptp_toolbox::data::operator<<(std::ostream& os, const bytes_data& data) {
    os << data.to_string();
    return os;
}

std::istream& ptp_toolbox::data::operator>>(std::istream& input, bytes_data& data) {
