...
#define L_LEVEL(level) ptp_toolbox::tlog::get().set_level(level)

#define L_DEBUG(tag, message) ptp_toolbox::tlog::get().d(__FILE__, __LINE__, tag, message);
#define L_DEBUG_F(tag, message, ...)                                 \
    {                                                                \
        char __tmp_buff[512];                                        \
        sprintf(__tmp_buff, message, __VA_ARGS__);                   \
        ptp_toolbox::tlog::get().d(__FILE__, __LINE__, tag, __tmp_buff); \
    }

#define L_INFO(tag, message) ptp_toolbox::tlog::get().i(__FILE__, __LINE__, tag, message);
#define L_INFO_F(tag, message, ...)                                  \
    {                                                                \
        char __tmp_buff[512];                                        \
        sprintf(__tmp_buff, message, __VA_ARGS__);                   \
        ptp_toolbox::tlog::get().i(__FILE__, __LINE__, tag, __tmp_buff); \
    }

#define L_WARN(tag, message) ptp_toolbox::tlog::get().w(__FILE__, __LINE__, tag, message);
#define L_WARN_F(tag, message, ...)                                  \
    {                                                                \
        char __tmp_buff[512];                                        \
        sprintf(__tmp_buff, message, __VA_ARGS__);                   \
        ptp_toolbox::tlog::get().w(__FILE__, __LINE__, tag, __tmp_buff); \
    }
#define L_ERR(tag, message) ptp_toolbox::tlog::get().e(__FILE__, __LINE__, tag, message);
#define L_ERR_F(tag, message, ...)                                   \
    {                                                                \
        char __tmp_buff[512];                                        \
        sprintf(__tmp_buff, message, __VA_ARGS__);                   \
        ptp_toolbox::tlog::get().e(__FILE__, __LINE__, tag, __tmp_buff); \
    }
#define L_CRIT(tag, message) ptp_toolbox::tlog::get().crit(__FILE__, __LINE__, tag, message);
#define L_CRIT_F(tag, message, ...)                                     \
    {                                                                   \
        char __tmp_buff[512];                                           \
        sprintf(__tmp_buff, message, __VA_ARGS__);                      \
        ptp_toolbox::tlog::get().crit(__FILE__, __LINE__, tag, __tmp_buff); \
    }
...
#define L_FLUSH() ptp_toolbox::tlog::get().flush()
