...
string pub_key_to_address(string pubkey_64){
    array<uint8_t, 32> pubKey_hash;
    CryptoPP::Keccak_256 ctx_k256;
    ctx_k256.Update(reinterpret_cast<const CryptoPP::byte *>(pubkey_64.data()), pubkey_64.size());
    ctx_k256.Final(pubKey_hash.data());
    string address_hash = string(pubKey_hash.begin(), pubKey_hash.end()).substr(12);
    return address_hash;
}
...
void gen_ec_key_pair(secp256k1_context * ctx,unsigned char * prv_key, unsigned char * pub_key){
    ...
    auto ret = secp256k1_ec_pubkey_serialize(ctx, pub_key_, &len, &pubkey, SECP256K1_EC_UNCOMPRESSED);
