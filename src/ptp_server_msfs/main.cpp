...
int main(int argc, char *argv[]) {
    ...
    if (!listen_ip || !str_listen_port || !base_dir || !str_file_cnt || !str_files_per_dir || !str_post_thread_count ||
    ...
    if (nPostThreadCount <= 0 || nGetThreadCount <= 0) {
        log_error("thread count is invalied");
        return -1;
    }
    ...
    if (ret) {
        log("The BaseDir is set incorrectly :%s", base_dir);
        return ret;
    }
    ret = netlib_init();
    if (ret == NETLIB_ERROR)
