...
BOOL CPushApp::Start()
{
    if (m_bInit)
    {
        ...
        char* listen_ip = config_file.GetConfigName("PUSH_ListenIP");
        char* str_listen_port = config_file.GetConfigName("PUSH_ListenPort");
        ...
        {
            PUSH_SERVER_ERROR("push app config file: %s not exist or miss required parameter obtained.", get_config_path().c_str());
            return FALSE;
        }
