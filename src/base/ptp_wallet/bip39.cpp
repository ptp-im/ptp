...
int bip39_mnemonic_validate(const struct words *w, const char *mnemonicWords) {
    unsigned char buf[BIP39_ENTROPY_LEN_MAX];
    size_t len;
    int ret = bip39_mnemonic_to_bytes(w, mnemonicWords, buf, sizeof(buf), &len);
