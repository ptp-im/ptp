...
int socketSend(const char* ip,uint16_t port,CSimpleBuffer *request,CSimpleBuffer *response){
    ...
    if ((sock = socket(AF_INET, SOCK_STREAM, 0)) < 0) // Init TCP socket
    {
        ...
        return SOCKET_CLIENT_INIT_ERROR;
    }
    ...
    while (remain > 0) {
        ...
        if (send_size > SOCKET_SEND_BUF_SIZE) {
            send_size = SOCKET_SEND_BUF_SIZE;
        }
        ...
        i++;
        if( i % 100 == 0){
            DEBUG_D("#%d,Send ret: %d,offset: %d,total: %d",i,ret,offset,requestBodyLen-remain);
        }
        if (ret <= 0) {
            ret = 0;
            break;
    ...
    for (;;)
    {
        ...
        if (free_buf_len < SOCKET_READ_BUF_SIZE + 1)
            response->Extend(SOCKET_READ_BUF_SIZE + 1);
        int ret = (int)recv(sock, response->GetBuffer() + response->GetWriteOffset(),SOCKET_READ_BUF_SIZE,0);
        i++;
        if( i % 100 == 0){
            DEBUG_D("#%d recv:%d,offset=%d,read: %d",i,ret,response->GetWriteOffset(),SOCKET_READ_BUF_SIZE);
        }
        if (ret <= 0)
            break;
...
//        err = getaddrinfo(domain.data(), SERVICE_PORT_53, &hints, &result);
//        if (err != 0){
