...
bool CGroupModel::removeGroup(uint32_t nUserId, uint32_t nGroupId, list<uint32_t>& lsCurUserId)
{
    ...
    if(pDBConn)
    {
        string strSql = "select creator from bd_im_group where id="+int2string(nGroupId);
        ...
        if(pResultSet)
        {
            ...
            {
                //设置群组不可用。
                strSql = "update bd_im_groupset status=0 where id="+int2string(nGroupId);
        ...
        if (bRet) {
            strSql = "select userId from bd_im_group_member where groupId="+int2string(nGroupId);
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if(pResultSet)
            {
                while (pResultSet->Next()) {
   ...
   if (!mapGroupId.empty())
    {
       CDBManager* pDBManager = CDBManager::getInstance();
        ...
        if (pDBConn)
        {
            ...
            string strSql = "select * from bd_im_group where id in (" + strClause  + ") order by updated desc";
            CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
            if(pResultSet)
            {
                while (pResultSet->Next()) {
...
bool CGroupModel::insertNewGroup(uint32_t nUserId, const string& strGroupName, const string& strGroupAvatar, uint32_t nGroupType, uint32_t nMemberCnt, uint32_t& nGroupId)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into bd_im_group(`name`, `avatar`, `creator`, `type`,`userCnt`, `status`, `version`, `lastChated`, `updated`, `created`) "\
        "values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
...
bool CGroupModel::insertNewMember(uint32_t nGroupId, set<uint32_t>& setUsers)
{
    ...
    if(nGroupId != INVALID_VALUE &&  nUserCnt > 0)
    {
        ...
        if (pDBConn)
        {
            ...
            string strSql = "select userId from bd_im_group_member where groupId=" + int2string(nGroupId) + " and userId in (" + strClause + ")";
            ...
            pDBConn = pDBManager->GetDBConn("bd_master");
            if (pDBConn)
            {
                ...
                if (pCacheConn)
                {
                    ...
                    if (!setHasUser.empty())
                    {
                        ...
                        strSql = "update bd_im_group_member set status=0, updated="+int2string(nCreated)+" where groupId=" + int2string(nGroupId) + " and userId in (" + strClause + ")";
                        pDBConn->ExecuteUpdate(strSql.c_str());
                    }
                    strSql = "insert into bd_im_group_member(`groupId`, `userId`, `status`, `created`, `updated`) values\
                    (?,?,?,?,?)";
                    ...
                    if(nIncMemberCnt != 0)
                    {
                        strSql = "update bd_im_group set userCnt=userCnt+" + int2string(nIncMemberCnt) + " where id="+int2string(nGroupId);
                        pDBConn->ExecuteUpdate(strSql.c_str());
                    }
...
void CGroupModel::getUserGroupIds(uint32_t nUserId, list<uint32_t>& lsGroupId, uint32_t nLimited)
{
    ...
    if(pDBConn)
    {
        string strSql ;
        if (nLimited != 0) {
            strSql = "select groupId from bd_im_group_member where userId=" + int2string(nUserId) + " and status = 0 order by updated desc, id desc limit " + int2string(nLimited);
        }
        else
        {
            strSql = "select groupId from bd_im_group_member where userId=" + int2string(nUserId) + " and status = 0 order by updated desc, id desc";
        }
...
void CGroupModel::getGroupVersion(list<uint32_t> &lsGroupId, list<IM::BaseDefine::GroupVersionInfo> &lsGroup, uint32_t nGroupType)
{
    if(!lsGroupId.empty())
    {
        ...
        if(pDBConn)
        {
            ...
            string strSql = "select id,version from bd_im_group where id in (" +  strClause  + ")";
            if(0 != nGroupType)
            {
...
bool CGroupModel::hasModifyPermission(uint32_t nUserId, uint32_t nGroupId, IM::BaseDefine::GroupModifyType nType)
{
    ...
    if(pDBConn)
    {
        string strSql = "select creator, type from bd_im_group where id="+ int2string(nGroupId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
...
bool CGroupModel::removeMember(uint32_t nGroupId, set<uint32_t> &setUser, list<uint32_t>& lsCurUserId)
{
    ...
    if(pDBConn)
    {
        ...
        if (pCacheConn)
        {
            ...
            string strSql = "update bd_im_group_member set status=1 where  groupId =" + int2string(nGroupId) + " and userId in(" + strClause + ")";
            pDBConn->ExecuteUpdate(strSql.c_str());
...
void CGroupModel::updateGroupChat(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        ...
        string strSql = "update bd_im_groupset lastChated=" + int2string(nNow) + " where id=" + int2string(nGroupId);
        pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
...
//    if(pDBConn)
//    {
//        string strSql = "select id from bd_im_group where id=" + int2string(nGroupId)+" and status=0";
//        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
...
bool CGroupModel::incGroupVersion(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        string strSql = "update bd_im_groupset version=version+1 where id="+int2string(nGroupId);
...
void CGroupModel::clearGroupMember(uint32_t nGroupId)
{
    ...
    if(pDBConn)
    {
        string strSql = "delete from bd_im_group_member where groupId="+int2string(nGroupId);
        pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
