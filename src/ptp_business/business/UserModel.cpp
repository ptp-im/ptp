...
void CUserModel::getChangedId(uint32_t& nLastTime,uint32_t&user_id, list<uint32_t> &lsIds)
{
    ...
    if (pDBConn)
    {
        ...
        if(nLastTime == 0)
        {
            strSql = "select id, updated from bd_im_user where status != 3 and id in (select f_user_id from bd_im_user_friend where user_id="+ int2string(user_id)+" and status = 1) ";
        }
        else
        {
            strSql = "select id, updated from bd_im_user where updated >= " + int2string(nLastTime)+" and id in (select f_user_id from bd_im_user_friend where user_id="+ int2string(user_id)+" and status = 1) ";
        }
...
void CUserModel::getUsers(list<uint32_t> lsIds, list<IM::BaseDefine::UserInfo> &lsUsers)
{
    ...
    if (pDBConn)
    {
        ...
        string  strSql = "select * from bd_im_user where id in (" + strClause + ")";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                cUser.set_user_nick_name(pResultSet->GetString("nick_name"));
                cUser.set_user_domain(pResultSet->GetString("domain"));
                cUser.set_user_real_name(pResultSet->GetString("user_name"));
...
bool CUserModel::getUser(uint32_t nUserId, DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        string strSql = "select * from bd_im_user where id="+int2string(nUserId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next())
            {
                ...
                cUser.strNick = pResultSet->GetString("nick_name");
                cUser.strDomain = pResultSet->GetString("domain");
                cUser.strName = pResultSet->GetString("user_name");
...
bool CUserModel::updateUser(DBUserInfo_t &cUser)
{
    bool bRet = false;
    CDBManager* pDBManager = CDBManager::getInstance();
    CDBConn* pDBConn = pDBManager->GetDBConn("bd_master");
    if (pDBConn)
    {
        uint32_t nNow = (uint32_t)time(NULL);
        string strSql = "update bd_im_user set `sex`=" + int2string(cUser.nSex)+ ", `nick_name`='" + cUser.strNick +"', `domain`='"+ cUser.strDomain + "', `phone`='" + cUser.strTel + "', `email`='" + cUser.strEmail+ "', `avatar`='" + cUser.strAvatar + "', `sign_info`='" + cUser.sign_info +"', `departId`='" + int2string(cUser.nDeptId) + "', `status`=" + int2string(cUser.nStatus) + ", `updated`="+int2string(nNow) + " where id="+int2string(cUser.nId);
    ...
    {
        log("no db connection for bd_master");
    }
    return bRet;
}
...
bool CUserModel::insertUser(DBUserInfo_t &cUser)
{
    ...
    if (pDBConn)
    {
        string strSql = "insert into bd_im_user(`id`,`sex`,`nick_name`,`domain`,`user_name`,`phone`,`email`,`avatar`,`sign_info`,`departId`,`status`,`created`,`updated`) values(?,?,?,?,?,?,?,?,?,?,?,?)";
...
bool CUserModel::updateUserAvatarUrl(uint32_t user_id, const string& avatar_url) {

    bool rv = false;
    CDBManager* db_manager = CDBManager::getInstance();
    CDBConn* db_conn = db_manager->GetDBConn("bd_master");
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update bd_im_user set `avatar`='" + avatar_url + "', `updated`=" + int2string(now) + " where id="+int2string(user_id);
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
            log("updateUserAvatarUrl: update failed:%s", str_sql.c_str());
        }else{
        ...
        log("updateUserAvatarUrl: no db connection for bd_master");
    }
    return rv;
}
...
bool CUserModel::updateUserSignInfo(uint32_t user_id, const string& sign_info) {
    ...
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update bd_im_user set `sign_info`='" + sign_info + "', `updated`=" + int2string(now) + " where id="+int2string(user_id);
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
        ...
        db_manager->RelDBConn(db_conn);
            ...
            log("updateUserSignInfo: no db connection for bd_master");
            }
    return rv;
    }
...
bool CUserModel::getUserSingInfo(uint32_t user_id, string* sign_info) {
    bool rv = false;
    CDBManager* db_manager = CDBManager::getInstance();
    CDBConn* db_conn = db_manager->GetDBConn("bd_slave");
    if (db_conn) {
        string str_sql = "select sign_info from bd_im_user where id="+int2string(user_id);
...
bool CUserModel::updatePushShield(uint32_t user_id, uint32_t shield_status) {
    ...
    if (db_conn) {
        uint32_t now = (uint32_t)time(NULL);
        string str_sql = "update bd_im_user set `push_shield_status`="+ int2string(shield_status) + ", `updated`=" + int2string(now) + " where id="+int2string(user_id);
        rv = db_conn->ExecuteUpdate(str_sql.c_str());
        if(!rv) {
        ...
        log("updatePushShield: no db connection for bd_master");
    }
...
bool CUserModel::getPushShield(uint32_t user_id, uint32_t* shield_status) {
    ...
    CDBConn* db_conn = db_manager->GetDBConn("bd_slave");
    if (db_conn) {
        string str_sql = "select push_shield_status from bd_im_user where id="+int2string(user_id);
        CResultSet* result_set = db_conn->ExecuteQuery(str_sql.c_str());
        if(result_set) {
            if (result_set->Next()) {
        ...
        log("getPushShield: no db connection for bd_slave");
    }
