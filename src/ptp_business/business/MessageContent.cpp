...
namespace DB_PROXY {
    void MsgReqProxy(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        do{
            ...
            CModelMsg::getInstance()->MsgReq(&msg, &msg_rsp, &msg_notify, error);
            list<uint32_t> group_members;
            CModelGroup::getInstance()->getGroupMembers(msg_rsp.group_id(),group_members);
            if(!group_members.empty()){
                for(auto it = group_members.begin();it!=group_members.end();it++){
                    msg_notify.add_notify_users(*it);
                    msg_notify.set_attach_data(msg.attach_data());
                    msg_notify.set_auth_uid(msg.auth_uid());
                }
            }
        } while(false);
        ...
        pPduResp->SetServiceId(PTP::Common::S_MSG);
        pPduResp->SetCommandId(PTP::Common::CID_MsgRes);
        CProxyConn::AddResponsePdu(conn_uuid, pPduResp);

        pPduNotify->SetPBMsg(&msg_notify);
        pPduNotify->SetSeqNum(0);
        pPduNotify->SetServiceId(PTP::Common::S_MSG);
        pPduNotify->SetCommandId(PTP::Common::CID_MsgNotify);
    ...
    void MsgReadAckReqProxy(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        do{
            ...
            CModelMsg::getInstance()->MsgReadAckReq(&msg, &msg_rsp, error);

            list<uint32_t> group_members;
            CModelGroup::getInstance()->getGroupMembers(msg_rsp.group_id(),group_members);
            if(!group_members.empty()){
                for(auto it = group_members.begin();it!=group_members.end();it++){
                    msg_rsp.add_notify_users(*it);
                    msg_rsp.set_attach_data(msg.attach_data());
                    msg_rsp.set_auth_uid(msg.auth_uid());
                }
            }
        ...
        pPduResp->SetSeqNum(pPdu->GetSeqNum());
        pPduResp->SetServiceId(PTP::Common::S_MSG);
        pPduResp->SetCommandId(PTP::Common::CID_MsgReadNotify);
        CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
    }
    ...
    void MsgGetMaxIdReqProxy(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        do{
            ...
            CModelMsg::getInstance()->MsgGetMaxIdReq(&msg, &msg_rsp, error);
        } while(false);
        ...
        pPduResp->SetSeqNum(pPdu->GetSeqNum());
        pPduResp->SetServiceId(PTP::Common::S_MSG);
        pPduResp->SetCommandId(PTP::Common::CID_MsgGetMaxIdRes);
        CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
    }
    ...
    void MsgUnNotifyProxy(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        do{
            ...
            CModelMsg::getInstance()->MsgUnNotify(&msg);
        } while(false);
    }
    ...
    void MsgGetByIdsReqProxy(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        do{
            ...
            CModelMsg::getInstance()->MsgGetByIdsReq(&msg, &msg_rsp, error);
        } while(false);
        ...
        pPduResp->SetSeqNum(pPdu->GetSeqNum());
        pPduResp->SetServiceId(PTP::Common::S_MSG);
        pPduResp->SetCommandId(PTP::Common::CID_MsgGetByIdsRes);
        CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
    }
