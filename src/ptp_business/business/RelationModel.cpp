...
uint32_t CRelationModel::getRelationId(uint32_t nUserAId, uint32_t nUserBId, bool bAdd)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "select id from bd_im_relationship where smallId=" + int2string(nSmallId) + " and bigId="+ int2string(nBigId) + " and status = 0";
...
uint32_t CRelationModel::addRelation(uint32_t nSmallId, uint32_t nBigId)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "select id from bd_im_relationship where smallId=" + int2string(nSmallId) + " and bigId="+ int2string(nBigId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet && pResultSet->Next())
        {
            nRelationId = pResultSet->GetInt("id");
            strSql = "update bd_im_relationship set status=0, updated=" + int2string(nTimeNow) + " where id=" + int2string(nRelationId);
        ...
        {
            strSql = "insert into bd_im_relationship (`smallId`,`bigId`,`status`,`created`,`updated`) values(?,?,?,?,?)";
...
bool CRelationModel::updateRelation(uint32_t nRelationId, uint32_t nUpdateTime)
{
    ...
    if (pDBConn)
    {
        string strSql = "update bd_im_relationship set `updated`="+int2string(nUpdateTime) + " where id="+int2string(nRelationId);
...
bool CRelationModel::removeRelation(uint32_t nRelationId)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "update bd_im_relationship set status = 1, updated="+int2string(nNow)+" where id=" + int2string(nRelationId);
        bRet = pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
