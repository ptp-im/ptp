...
void CDepartModel::getChgedDeptId(uint32_t& nLastTime, list<uint32_t>& lsChangedIds)
{
    ...
    if (pDBConn)
    {
        string strSql = "select id, updated from bd_im_depart where updated > " + int2string(nLastTime);
...
void CDepartModel::getDepts(list<uint32_t>& lsDeptIds, list<IM::BaseDefine::DepartInfo>& lsDepts)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "select * from bd_im_depart where id in ( " + strClause + " )";
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next()) {
...
void CDepartModel::getDept(uint32_t nDeptId, IM::BaseDefine::DepartInfo& cDept)
{
    ...
    if (pDBConn)
    {
        string strSql = "select * from bd_im_depart where id = " + int2string(nDeptId);
        CResultSet* pResultSet = pDBConn->ExecuteQuery(strSql.c_str());
        if(pResultSet)
        {
            while (pResultSet->Next()) {
