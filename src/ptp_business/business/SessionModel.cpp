...
void CSessionModel::getRecentSession(uint32_t nUserId, uint32_t lastTime, list<IM::BaseDefine::ContactSessionInfo>& lsContact)
{
    ...
    if (pDBConn)
    {
        string strSql = "select * from bd_im_session where userId = " + int2string(nUserId) + " and status = 0 and updated >" + int2string(lastTime) + " order by updated desc limit 100";
...
uint32_t CSessionModel::getSessionId(uint32_t nUserId, uint32_t nPeerId, uint32_t nType, bool isAll)
{
    ...
    if(pDBConn)
    {
        string strSql;
        if (isAll) {
            strSql= "select id from bd_im_session where userId=" + int2string(nUserId) + " and peerId=" + int2string(nPeerId) + " and type=" + int2string(nType);
        }
        else
        {
            strSql= "select id from bd_im_session where userId=" + int2string(nUserId) + " and peerId=" + int2string(nPeerId) + " and type=" + int2string(nType) + " and status = 0";
        }
...
bool CSessionModel::updateSession(uint32_t nSessionId, uint32_t nUpdateTime)
{
    ...
    if (pDBConn)
    {
        string strSql = "update bd_im_session set `updated`="+int2string(nUpdateTime) + " where id="+int2string(nSessionId);
...
bool CSessionModel::removeSession(uint32_t nSessionId)
{
    ...
    if (pDBConn)
    {
        ...
        string strSql = "update bd_im_session set status = 1, updated="+int2string(nNow)+" where id=" + int2string(nSessionId);
        bRet = pDBConn->ExecuteUpdate(strSql.c_str());
        pDBManager->RelDBConn(pDBConn);
    }
    else
...
uint32_t CSessionModel::addSession(uint32_t nUserId, uint32_t nPeerId, uint32_t nType)
{
    ...
    if (pDBConn)
    {
        ...
        {
            string strSql = "update bd_im_session set status=0, updated=" + int2string(nTimeNow) + " where id=" + int2string(nSessionId);
        ...
        {
            string strSql = "insert into bd_im_session (`userId`,`peerId`,`type`,`status`,`created`,`updated`) values(?,?,?,?,?,?)";
