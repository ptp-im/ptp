...
void setGlobalAuthKey(string val){
    GlobalAuthKey = std::move(val);
}

void setGlobalAuthToken(string val){
    GlobalAuthToken = std::move(val);
}

string getGlobalAuthKey(){
     return GlobalAuthKey;
}

string getGlobalAuthToken(){
...
int CacheConn::Init()
{
	...
	if (!m_pContext || m_pContext->err) {
		if (m_pContext) {
			DEBUG_I("redisConnect failed: %s,%s:%d", m_pContext->errstr,m_pCachePool->GetServerIP(),m_pCachePool->GetServerPort());
			redisFree(m_pContext);
			m_pContext = NULL;
		} else {
			DEBUG_I("redisConnect failed");
		}

		return 1;
	}
    redisReply* reply1 = (redisReply *)redisCommand(m_pContext, "AUTH %s", m_pCachePool->GetAuth());
    if (reply1 && (reply1->type == REDIS_REPLY_STATUS) && (strncmp(reply1->str, "OK", 2) == 0)) {
        ...
        if (reply && (reply->type == REDIS_REPLY_STATUS) && (strncmp(reply->str, "OK", 2) == 0)) {
            ...
            DEBUG_I("select cache db failed");
            return 2;
        }
    } else {
        DEBUG_I("AUTH cache db failed");
        return 2;
    }
}
...
string CacheConn::get(string key)
{
	...
	if (!reply) {
		DEBUG_I("redisCommand failed:%s", m_pContext->errstr);
		redisFree(m_pContext);
		m_pContext = NULL;
		return value;
	}
...
long CacheConn::incrBy(string key, long value)
{
    ...
    if(!reply)
    {
        DEBUG_I("redis Command failed:%s", m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
        return -1;
    }
...
bool CacheConn::exec(list<string>* argv_list ,list<string>* ret_list){

    if (Init() || argv_list->size() == 0) {
        return false;
    }
    ...
    if (reply->type == REDIS_REPLY_ARRAY && ret_list != nullptr) {
        for (size_t i = 0; i < reply->elements; i++) {
            redisReply* value_reply = reply->element[i];
            string value(value_reply->str, value_reply->len);
...
bool CacheConn::cmd(string cmd_name,string key, list<string>* argv_list, list<string>* ret_list )
{
    if (Init()) {
        return false;
    }

    int argc = argv_list ? argv_list->size() + 2 : 2;
    const char** argv = new const char* [argc];
    if (!argv) {
        return false;
    }

    argv[0] = cmd_name.c_str();
    argv[1] = key.c_str();
    if(argv_list){
        int i = 2;
        for (list<string>::iterator it = argv_list->begin(); it != argv_list->end(); it++) {
...
///////////////
CachePool::CachePool(const char* pool_name, const char* server_ip, int server_port, const char* auth, int db_num, int max_conn_cnt)
{
	...
	while (m_free_list.empty()) {
		if (m_cur_conn_cnt >= m_max_conn_cnt) {
			...
			if (ret) {
				...
				m_cur_conn_cnt++;
				DEBUG_I("new cache connection: %s, conn_cnt: %d", m_pool_name.c_str(), m_cur_conn_cnt);
			}
		}
	}
...
int CacheManager::Init()
{
	...
	if (!cache_instances) {
		DEBUG_I("not configure CacheIntance");
		return 1;
	}
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
        ...
        char* str_cache_auth = config_file.GetConfigName(auth);
		char* str_cache_db = config_file.GetConfigName(db);
        char* str_max_conn_cnt = config_file.GetConfigName(maxconncnt);
		if (!cache_host || !str_cache_port|| !str_cache_auth || !str_cache_db || !str_max_conn_cnt) {
			DEBUG_I("not configure cache instance: %s", pool_name);
			return 2;
		}

		CachePool* pCachePool = new CachePool(pool_name, cache_host, atoi(str_cache_port),str_cache_auth,
				atoi(str_cache_db), atoi(str_max_conn_cnt));
		if (pCachePool->Init()) {
			DEBUG_I("Init cache pool failed");
			return 3;
		}
