...
namespace COMMAND {
    void MsgGetMaxIdReqCmd(CImPdu* pPdu, uint32_t conn_uuid){
        log_debug("MsgGetMaxIdReq start...");
        ...
        if(error!= NO_ERROR){
            ...
            pdu.SetCommandId(CID_MsgGetMaxIdRes);
            pdu.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu);
            //CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        log_debug("MsgGetMaxIdReq end...");
    }
    
    void MsgGetMaxIdResCmd(CImPdu* pPdu){
        log_debug("MsgGetMaxIdRes start...");
        ...
        while (true){
            ...
            pdu_rsp.SetCommandId(PTP::Common::CID_MsgGetMaxIdRes);
            pdu_rsp.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu_rsp);
            break;
        }
        log_debug("MsgGetMaxIdRes end...");
    }
};
