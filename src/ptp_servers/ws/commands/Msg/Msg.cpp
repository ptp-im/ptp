...
namespace COMMAND {
    void MsgReqCmd(CImPdu* pPdu, uint32_t conn_uuid){
        ...
        if(error!= NO_ERROR){
            ...
            pdu.SetCommandId(CID_MsgRes);
            pdu.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu);
            //CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        log_debug("MsgReq end...");
    }
    
    void MsgResCmd(CImPdu* pPdu){
        log_debug("MsgRes start...");
        PTP::Msg::MsgRes msg;
        ...
        while (true){
            ...
            pdu_rsp.SetCommandId(PTP::Common::CID_MsgRes);
            pdu_rsp.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu_rsp);
            break;
        }
        log_debug("MsgRes end...");
    }
};
