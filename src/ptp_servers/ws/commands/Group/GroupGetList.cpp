...
namespace COMMAND {
    void GroupGetListReqCmd(CImPdu* pPdu, uint32_t conn_uuid){
        log_debug("GroupGetListReq start...");
        ...
        if(error!= NO_ERROR){
            ...
            pdu.SetCommandId(CID_GroupGetListRes);
            pdu.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu);
            //CProxyConn::AddResponsePdu(conn_uuid, pPduResp);
        }
        log_debug("GroupGetListReq end...");
    }
    
    void GroupGetListResCmd(CImPdu* pPdu){
        log_debug("GroupGetListRes start...");
        ...
        while (true){
            ...
            pdu_rsp.SetCommandId(PTP::Common::CID_GroupGetListRes);
            pdu_rsp.SetSeqNum(pPdu->GetSeqNum());
            pMsgConn->SendPdu(&pdu_rsp);
            break;
        }
        log_debug("GroupGetListRes end...");
    }
};
