 ...
 *
 *   filename：  GroupChangeMemberNotify.cpp
 *   author：    Barry
...
namespace COMMAND {
    void GroupChangeMemberNotifyCmd(CImPdu* pPdu){
        log_debug("GroupChangeMemberNotify start...");
        ...
        while (true){
            ...
            // pdu_rsp.SetCommandId(PTP::Common::CID_GroupChangeMemberNotify);
            // pdu_rsp.SetSeqNum(pPdu->GetSeqNum());
            // pMsgConn->SendPdu(&pdu_rsp);
            break;
        }
        log_debug("GroupChangeMemberNotify end...");
    }
};
