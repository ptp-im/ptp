...
int FileManager::initDir() {
		...
		if (!isExist) {
			...
			if (ret) {
				DEBUG_D("The dir[%s] set error for code[%d], \
		...
		for (int i = 0; i <= FIRST_DIR_MAX; i++) {
			...
			if (code && (errno != EEXIST)) {
				DEBUG_D("Create dir[%s] error[%d]", tmp.c_str(), errno);
				return -1;
			}
			for (int j = 0; j <= SECOND_DIR_MAX; j++) {
		    	...
		    	if (code && (errno != EEXIST)) {
					DEBUG_D("Create dir[%s] error[%d]", tmp2.c_str(), errno);
					return -1;
		    	}
...
int FileManager::uploadFile(const char *type, const void* content, u32 size, 
	...
	if (size > MAX_FILE_SIZE_PER_FILE) {
		DEBUG_D("File size[%d] should less than [%d]", size,
...
int FileManager::getRelatePathByUrl(const string &url, string &path) {
	...
	if (string::npos == pos) {
		DEBUG_D("Url [%s] format illegal.",url.c_str());
		return -1;
	}
	path = url.substr(pos);
	return 0;
...
int FileManager::getAbsPathByUrl(const string &url, string &path) {
	...
	if (getRelatePathByUrl(url, relate)) {
		DEBUG_D("Get path from url[%s] error", url.c_str());
		return -1;
	}
...
FileManager::getOrCreateEntry(const std::string& url, bool create) {
	...
	if (it != m_map.end()) {
		DEBUG_D("the map has the file while url:%s", url.c_str());
		m_cs.Leave();
		return it->second;
	}
	if (!create) {
	...
	if (getAbsPathByUrl(url, path)) {
		DEBUG_D("Get abs path from url[%s] error", url.c_str());
		m_cs.Leave();
		return NULL;
	}
	...
	if (ret) {
		DEBUG_D("read file error while url:%s", url.c_str());
		delete e;
		e = NULL;
		delete tmpFile;
		tmpFile = NULL;
	...
	if (result.second == false) {
		DEBUG_D("Insert url[%s] to file map error", url.c_str());
		delete e;
		e = NULL;
	}
	updateMapCache();
...
int FileManager::downloadFileByUrl(char *url, void *buf, u32 *size) {
	...
	if (!en) {
		DEBUG_D("download file error, while url:%s", url);
		return -1;
	}
...
void FileManager::releaseFileCache(const std::string& url) {
	...
	if (!entry) {
		DEBUG_D("map has not the url::%s", url.c_str());
        m_cs.Leave();
		return;
	}
