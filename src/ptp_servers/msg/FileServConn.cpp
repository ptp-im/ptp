...
{
	DEBUG_I("Connecting to FileServer %s:%d ", server_ip, server_port);
...
{
	DEBUG_I("connect to file server success ");
	m_bOpen = true;
...
{
	DEBUG_I("onclose from file server handle=%d ", m_handle);
	Close();
}
...
void CFileServConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		DEBUG_I("conn to file server timeout ");
		Close();
	}
}
...
void CFileServConn::_HandleFileMsgTransRsp(CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleFileMsgTransRsp, result: %u, from_user_id: %u, to_user_id: %u, file_name: %s, \
...
void CFileServConn::_HandleFileServerIPRsp(CImPdu* pPdu)
{
    ...
    for (uint32_t i = 0; i < ip_addr_cnt ; i++)
    {
        ...
        DEBUG_I("_HandleFileServerIPRsp -> %s : %d ", ip_addr.ip().c_str(), ip_addr.port());
