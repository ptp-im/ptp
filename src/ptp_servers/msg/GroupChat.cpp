...
void CGroupChat::HandleClientGroupNormalRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
    ...
    DEBUG_I("HandleClientGroupNormalRequest, user_id=%u. ", user_id);
...
void CGroupChat::HandleGroupNormalResponse(CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleGroupNormalResponse, user_id=%u, group_cnt=%u. ", user_id, group_cnt);
...
void CGroupChat::HandleClientGroupInfoRequest(CImPdu *pPdu, CMsgConn* pFromConn)
{
    ...
    uint32_t user_id = pFromConn->GetUserId();
    uint32_t group_cnt = msg.group_version_list_size();
    DEBUG_I("HandleClientGroupInfoRequest, user_id=%u, group_cnt=%u. ", user_id, group_cnt);
...
void CGroupChat::HandleGroupInfoResponse(CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleGroupInfoResponse, user_id=%u, group_cnt=%u. ", user_id, group_cnt);
    ...
    if (pduAttachData.GetPduLength() > 0 && group_cnt > 0)
    {
        ...
        DEBUG_I("GroupInfoRequest is send by server, group_id=%u ", group_id);
        ...
        {
            DEBUG_I("user_id=%u is not in group, group_id=%u. ", user_id, group_id);
            return;
        }
...
void CGroupChat::HandleGroupMessage(CImPdu* pPdu)
{
    ...
    if (msg_id == 0) {
        DEBUG_I("HandleGroupMsg, write db failed, %u->%u. ", from_user_id, to_group_id);
        return;
    }
    uint8_t msg_type = msg.msg_type();
    CDbAttachData attach_data((uchar_t*)msg.attach_data().c_str(), msg.attach_data().length());

    DEBUG_I("HandleGroupMsg, %u->%u, msg id=%u. ", from_user_id, to_group_id, msg_id);
...
void CGroupChat::HandleGroupMessageBroadcast(CImPdu *pPdu)
{
    ...
    string msg_data = msg.msg_data();
    uint32_t msg_id = msg.msg_id();
    DEBUG_I("HandleGroupMessageBroadcast, %u->%u, msg id=%u. ", from_user_id, to_group_id, msg_id);
...
void CGroupChat::HandleClientGroupCreateRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
    ...
    if (group_type == IM::BaseDefine::GROUP_TYPE_NORMAL) {
        DEBUG_I("HandleClientGroupCreateRequest, create normal group failed, req_id=%u, group_name=%s. ", req_user_id, group_name.c_str());
        return;
    }
	string group_avatar = msg.group_avatar();
	uint32_t user_cnt = msg.member_id_list_size();
	DEBUG_I("HandleClientGroupCreateRequest, req_id=%u, group_name=%s, avatar_url=%s, user_cnt=%u ",
...
void CGroupChat::HandleGroupCreateResponse(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleGroupCreateResponse, req_id=%u, result=%u, group_id=%u, group_name=%s, member_cnt=%u. ", user_id, result, group_id, group_name.c_str(), user_cnt);
...
void CGroupChat::HandleClientGroupChangeMemberRequest(CImPdu* pPdu, CMsgConn* pFromConn)
{
	...
	uint32_t group_id = msg.group_id();
	uint32_t user_cnt = msg.member_id_list_size();
	DEBUG_I("HandleClientChangeMemberReq, change_type=%u, req_id=%u, group_id=%u, user_cnt=%u ",
...
void CGroupChat::HandleGroupChangeMemberResponse(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleChangeMemberResp, change_type=%u, req_id=%u, group_id=%u, result=%u, chg_usr_cnt=%u, cur_user_cnt=%u. ",
...
void CGroupChat::HandleGroupChangeMemberBroadcast(CImPdu* pPdu)
{
	...
	uint32_t chg_user_cnt = msg.chg_user_id_list_size();
    uint32_t cur_user_cnt = msg.cur_user_id_list_size();
	DEBUG_I("HandleChangeMemberBroadcast, change_type=%u, group_id=%u, chg_user_cnt=%u, cur_user_cnt=%u. ", change_type, group_id, chg_user_cnt, cur_user_cnt);
...
void CGroupChat::HandleClientGroupShieldGroupRequest(CImPdu *pPdu, CMsgConn *pFromConn)
{
    ...
    uint32_t group_id = msg.group_id();
    uint32_t user_id = pFromConn->GetUserId();
    DEBUG_I("HandleClientGroupShieldGroupRequest, user_id: %u, group_id: %u, shield_status: %u. ",
...
void CGroupChat::HandleGroupShieldGroupResponse(CImPdu *pPdu)
{
    ...
    uint32_t user_id = msg.user_id();
    uint32_t group_id = msg.group_id();
    DEBUG_I("HandleGroupShieldGroupResponse, result: %u, user_id: %u, group_id: %u. ", result,
        user_id, group_id);
...
void CGroupChat::HandleGroupGetShieldByGroupResponse(CImPdu *pPdu)
{
    ...
    DEBUG_I("HandleGroupGetShieldByGroupResponse, shield_status_list_cnt: %u. ",
    ...
    for (uint32_t i = 0; i < shield_status_list_cnt; i++)
    {
        ...
        {
            DEBUG_I("user_id: %u shield group, group id: %u. ", shield_status.user_id(), shield_status.group_id());
        }
    }
    CImPdu pdu;
    pdu.SetPBMsg(&msg2);
