...
void imconn_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
    ...
    //DEBUG_D("msg=%d, handle=%d ", msg, handle);
	...
	case NETLIB_MSG_CLOSE:
		...
		break;
	default:
		DEBUG_E("!!!imconn_callback error msg: %d ", msg);
		break;
	}
...
CImConn::~CImConn()
{
	//DEBUG_D("CImConn::~CImConn, handle=%d ", m_handle);
}
...
//	++g_send_pkt_cnt;
	...
	while (remain > 0) {
		...
		//DEBUG_D("pdu send i#%d,len=%d,remain=%d,ret=%d,offset=%d,send_size=%d", i,len,remain,ret,offset,send_size);
...
void CImConn::OnWrite()
{
	...
	DEBUG_D("onWrite, remain=%d ", m_out_buf.GetWriteOffset());
}
