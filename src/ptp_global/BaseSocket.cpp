...
{
	//DEBUG_D("CBaseSocket::CBaseSocket\n");
...
CBaseSocket::~CBaseSocket()
{
//	DEBUG_D("CBaseSocket::~CBaseSocket, socket=%d\n", m_socket);
}
...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	{
		DEBUG_E("socket failed, err_code=%d\n", _GetErrorCode());
		return NETLIB_ERROR;
	}
...
{
	//DEBUG_D("CBaseSocket::Connect, server_ip=%s, port=%d", server_ip, port);
	...
	if (m_socket == INVALID_SOCKET)
	{
       DEBUG_D("socket failed, err_code=%d", _GetErrorCode());
	...
	{
       DEBUG_D("connect failed, err_code=%d", _GetErrorCode());
		closesocket(m_socket);
		return NETLIB_INVALID_HANDLE;
	}
...
#endif
			ret = 0;
DEBUG_D("socket send block fd=%d", m_socket);
		}
		else
		{
//           DEBUG_D("!!!send failed, error code: %d", err_code);
		}
	}
...
void CBaseSocket::SetSendBufSize(uint32_t send_size)
{
	...
	if (ret == SOCKET_ERROR) {
		DEBUG_D("set SO_SNDBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_SNDBUF, &size, &len);
	DEBUG_D("socket=%d send_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::SetRecvBufSize(uint32_t recv_size)
{
	...
	if (ret == SOCKET_ERROR) {
		DEBUG_D("set SO_RCVBUF failed for fd=%d", m_socket);
	}

	socklen_t len = 4;
	int size = 0;
	getsockopt(m_socket, SOL_SOCKET, SO_RCVBUF, &size, &len);
	DEBUG_D("socket=%d recv_buf_size=%d", m_socket, size);
}
...
void CBaseSocket::_SetNonblock(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       DEBUG_D("_SetNonblock failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetReuseAddr(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       DEBUG_D("_SetReuseAddr failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetNoDelay(SOCKET fd)
{
	...
	if (ret == SOCKET_ERROR)
	{
       DEBUG_D("_SetNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
...
void CBaseSocket::_SetAddr(const char* ip, const uint16_t port, sockaddr_in* pAddr)
{
	...
	if (pAddr->sin_addr.s_addr == INADDR_NONE)
	{
		...
		if (host == NULL)
		{
           DEBUG_D("gethostbyname failed, ip=%s", ip);
			return;
		}
...
void CBaseSocket::_AcceptNewSocket()
{
	...
	while ( (fd = accept(m_socket, (sockaddr*)&peer_addr, &addr_len)) != INVALID_SOCKET )
	{
		...
		DEBUG_D("AcceptNewSocket, socket=%d from %s:%d", fd, ip_str, port);
