...
CEventDispatch::CEventDispatch()
{
	...
	if (m_kqfd == -1)
	{
DEBUG_D("kqueue failed");
	}
#else
	m_epfd = epoll_create(1024);
	if (m_epfd == -1)
	{
DEBUG_D("epoll_create failed");
	}
#endif
}
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			DEBUG_D("select failed, error code: %d", GetLastError());
			Sleep(MIN_TIMER_DURATION);
		...
		{
			DEBUG_D("select return read count=%d", read_set.fd_count);
		...
		{
			DEBUG_D("select return write count=%d", write_set.fd_count);
		...
		{
			DEBUG_D("select return exception count=%d", excep_set.fd_count);
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
    ...
    while (running)
	{
		...
		{
			auto ev_fd = events[i].ident;
			CBaseSocket* pSocket = FindBaseSocket((int)ev_fd);
			if (!pSocket)
				continue;
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		DEBUG_D("epoll_ctl() failed, errno=%d", errno);
	}
}
