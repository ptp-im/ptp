...
string CHttpClient::UploadByteFile(const string &strUrl, string& content,const string &fileName,const string &fileType)
{
    if(strUrl.empty())
        return "";
    ...
    body.append(content.c_str(), content.size());    // image buffer
    ...
    if (CURLE_OK != res) {
        DEBUG_I("curl_easy_perform failed, res=%d", res);
        return "";
    }
    ...
    if (!reader.parse(strResp, value)) {
        DEBUG_I("json parse failed: %s", strResp.c_str());
        return "";
    }

    if (value["error_code"].isNull()) {
        DEBUG_I("no code in response %s", strResp.c_str());
        return "";
    }
    uint32_t nRet = value["error_code"].asUInt();
    if(nRet != 0)
    {
        DEBUG_I("upload faile:%u", nRet);
        return "";
    }
    return value["path"].asString();
}
...
//        DEBUG_I("curl_easy_perform failed, res=%d, ret=%u", res, retcode);
//    }
//    double nLen = 0;
