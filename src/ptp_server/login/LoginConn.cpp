...
void login_conn_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	...
	for (auto it = g_client_conn_map.begin(); it != g_client_conn_map.end(); ) {
		...
		auto* pConn = (CLoginConn*)it_old->second;
...
void CLoginConn::Close()
{
	if (m_handle != NETLIB_INVALID_HANDLE) {
		...
		if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
			...
			auto it = g_msg_serv_info.find(m_handle);
			if (it != g_msg_serv_info.end()) {
				msg_serv_info_t* pMsgServInfo = it->second;

				g_total_online_user_cnt -= pMsgServInfo->cur_conn_cnt;
				DEBUG_I("onclose from MsgServer: %s:%u ", pMsgServInfo->hostname.c_str(), pMsgServInfo->port);
				delete pMsgServInfo;
...
void CLoginConn::OnTimer(uint64_t curr_tick)
{
	if (m_conn_type == LOGIN_CONN_TYPE_CLIENT) {
		...
		if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
			DEBUG_I("connection to MsgServer timeout ");
			Close();
		}
	}
}
...
//            break;
        ...
        default:
            DEBUG_I("wrong msg, cmd id=%d ", pPdu->GetCommandId());
            break;
	}
}
//
...
//	g_total_online_user_cnt += pMsgServInfo->cur_conn_cnt;
//
//	DEBUG_I("MsgServInfo, ip_addr1=%s, ip_addr2=%s, port=%d, max_conn_cnt=%d, cur_conn_cnt=%d, "\
//		"hostname: %s. ",
...
//		}
//
//		DEBUG_I("%s:%d, cur_cnt=%u, total_cnt=%u ", pMsgServInfo->hostname.c_str(),
