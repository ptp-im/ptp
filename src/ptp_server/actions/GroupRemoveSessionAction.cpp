...
namespace ACTION_GROUP {
    void GroupRemoveSessionReqAction(CRequest* request){
        ...
        while (true){
            ...
            if(!request->IsBusinessConn()){
             ...
             request->Next(&msg,CID_GroupRemoveSessionReq,request->GetSeqNum());
            }else{
                ...
                request->SendResponseMsg(&msg_rsp,CID_GroupRemoveSessionRes,request->GetSeqNum());
            }
            break;
        }
        if(error!= NO_ERROR){
    ...
    void GroupRemoveSessionResAction(CRequest* request){
        ...
        // }
        // msg.set_error(error);
        // request->SendResponseMsg(&msg,CID_GroupRemoveSessionRes,request->GetSeqNum());
