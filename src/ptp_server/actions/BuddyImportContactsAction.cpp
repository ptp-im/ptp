...
namespace ACTION_BUDDY {
    void BuddyImportContactsReqAction(CRequest* request){
        ...
        while (true){
            ...
            if(!request->IsBusinessConn()){
             ...
             request->Next(&msg,CID_BuddyImportContactsReq,request->GetSeqNum());
            }else{
                ...
                request->SendResponseMsg(&msg_rsp,CID_BuddyImportContactsRes,request->GetSeqNum());
            }
            break;
        }
        if(error!= NO_ERROR){
    ...
    void BuddyImportContactsResAction(CRequest* request){
        ...
        // }
        // msg.set_error(error);
        // request->SendResponseMsg(&msg,CID_BuddyImportContactsRes,request->GetSeqNum());
