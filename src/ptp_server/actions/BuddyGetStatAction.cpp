...
namespace ACTION_BUDDY {
    void BuddyGetStatReqAction(CRequest* request){
        ...
        while (true){
            ...
            if(!request->IsBusinessConn()){
             ...
             request->Next(&msg,CID_BuddyGetStatReq,request->GetSeqNum());
            }else{
                ...
                request->SendResponseMsg(&msg_rsp,CID_BuddyGetStatRes,request->GetSeqNum());
            }
            break;
        }
        if(error!= NO_ERROR){
    ...
    void BuddyGetStatResAction(CRequest* request){
        ...
        // }
        // msg.set_error(error);
        // request->SendResponseMsg(&msg,CID_BuddyGetStatRes,request->GetSeqNum());
