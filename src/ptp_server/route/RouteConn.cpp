...
CUserInfo* GetUserInfo(uint32_t user_id)
{
    ...
    auto it = g_user_map.find(user_id);
...
void route_serv_timer_callback(void* callback_data, uint8_t msg, uint32_t handle, void* pParam)
{
	...
	for (auto it = g_route_conn_map.begin(); it != g_route_conn_map.end(); ) {
		...
		auto* pConn = (CRouteConn*)it_old->second;
		pConn->OnTimer(cur_time);
	}
}

void init_route_timer_callback()
{
...
{
	DEBUG_I("MsgServer onclose: handle=%d ", m_handle);
	Close();
}
...
void CRouteConn::OnTimer(uint64_t curr_tick)
{
	if (curr_tick > m_last_send_tick + SERVER_HEARTBEAT_INTERVAL)
    {
        ...
        ImPdu pdu;
        pdu.SetPBMsg(&msg,CID_HeartBeatNotify,0);
		SendPdu(&pdu);
	}

	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		DEBUG_I("message server timeout ");
		Close();
	}
}
...
//            break;
	...
	default:
		DEBUG_I("CRouteConn::HandlePdu, wrong cmd id: %d ", pPdu->GetCommandId());
		break;
	}
}
//
...
//
//	DEBUG_I("HandleOnlineUserInfo, user_cnt=%u ", user_count);
//
...
//	DEBUG_I("HandleUserStatusUpdate, status=%u, uid=%u, client_type=%u ", user_status, user_id, client_type);
//
...
//
//	DEBUG_I("HandleRoleSet, master=%u, handle=%u ", master, m_handle);
//	if (master == 1) {
//		m_bMaster = true;
//	} else {
//		m_bMaster = false;
...
//	DEBUG_I("HandleUserStatusReq, req_id=%u, query_count=%u ", request_id, query_count);
//
...
int run_ptp_server_route(int argc, char* argv[])
{
    bool isDebug = false;
    ...
    string server_name = "ptp_server_route";
    ...
    char* listen_ip_str = config_file.GetConfigName("ROUTE_ListenIP");
    char* listen_port_str = config_file.GetConfigName("ROUTE_ListenPort");
    ...
    DEBUG_I("%s start listen on: %s:%d",server_name.c_str(), listen_ip_str, listen_port);
    init_route_timer_callback();
