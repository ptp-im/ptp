...
void CFileHandler::HandleClientFileRequest(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleClientFileRequest, %u->%u, fileName: %s, trans_mode: %u.", from_id, to_id, file_name.c_str(), trans_mode);
    ...
    {
        DEBUG_I("HandleClientFileRequest, no file server.   ");
...
void CFileHandler::HandleClientFileHasOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleClientFileHasOfflineReq, req_id=%u   ", req_user_id);
    ...
    {
        DEBUG_I("warning no DB connection available ");
...
void CFileHandler::HandleClientFileAddOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    string file_name = msg.file_name();
    uint32_t file_size = msg.file_size();
    DEBUG_I("HandleClientFileAddOfflineReq, %u->%u, task_id: %s, file_name: %s, size: %u  ",
...
void CFileHandler::HandleClientFileDelOfflineReq(CMsgConn* pMsgConn, CImPdu* pPdu)
{
    ...
    uint32_t to_id = msg.to_user_id();
    string task_id = msg.task_id();
    DEBUG_I("HandleClientFileDelOfflineReq, %u->%u, task_id=%s ", from_id, to_id, task_id.c_str());
...
void CFileHandler::HandleFileHasOfflineRes(CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleFileHasOfflineRes, req_id=%u, file_cnt=%u ", req_user_id, file_cnt);
    ...
    {
        DEBUG_I("HandleFileHasOfflineRes, no file server. ");
    }
    if (pConn) {
        pPdu->SetPBMsg(&msg);
        pConn->SendPdu(pPdu);
...
void CFileHandler::HandleFileNotify(CImPdu* pPdu)
{
    ...
    DEBUG_I("HandleFileNotify, from_id: %u, to_id: %u, file_name: %s, task_id: %s, trans_mode: %u,\
