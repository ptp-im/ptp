...
//    char data[1024] = {};
    ...
    strcat(header, ("Content-Length: " + to_string(body_len) + "\r\n\r\n").c_str());


    char * data = new char[body_len + strlen( header)]();
    strcat(data, header);
    strcat(data, (char *)body);
    int len = strlen(data);
...
void CHttpConn::OnRead()
{
	...
	if (m_HttpParser.IsReadAll()) {
        ...
        if(strncmp(url.c_str(), "/api", 4) == 0 ){
            string content = m_HttpParser.GetBodyContent();
            CHttpQuery::DispatchQuery(url, content, this);
        }
        else if (strcmp(url.c_str(), "/favicon.ico") == 0)
        {
            Close();
        }
        else
