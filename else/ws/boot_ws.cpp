...
int boot_ws(int argc, char* argv[])
{
...
//    }
    ...
    uint32_t enable_business_async = false;
    char* enable_business_async_str = config_file.GetConfigName("WS_ENABLE_BUSINESS_ASYNC");
    if (enable_business_async_str) {
        if(atoi(enable_business_async_str) == 1){
            enable_business_async = true;
        }
    }
    set_enable_business_async(enable_business_async);
