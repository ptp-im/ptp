...
{
	DEBUG_I("Connecting to RouteServer %s:%d ", server_ip, server_port);
...
{
	DEBUG_I("connect to route server success ");
	m_bOpen = true;
...
{
	DEBUG_I("onclose from route server handle=%d ", m_handle);
	Close();
}
...
void CRouteServConn::OnTimer(uint64_t curr_tick)
{
	...
	if (curr_tick > m_last_recv_tick + SERVER_TIMEOUT) {
		DEBUG_I("conn to route server timeout ");
		Close();
	}
}
...
void CRouteServConn::HandlePdu(CImPdu* pPdu)
{
	switch (pPdu->GetCommandId()) {
        ...
        case CID_FILE_NOTIFY:
            ...
            break;
        default:
            DEBUG_I("unknown cmd id=%d ", pPdu->GetCommandId());
            break;
	}
}
...
void CRouteServConn::_HandleKickUser(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleKickUser, user_id=%u, client_type=%u, reason=%u. ", user_id, client_type, reason);
...
void CRouteServConn::_HandleStatusNotify(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleFriendStatusNotify, user_id=%u, status=%u ", user_stat.user_id(), user_stat.status());
...
//    }
	...
	DEBUG_I("HandleMsgData, %u->%u, msg_id=%u. ", from_user_id, to_user_id, msg_id);
...
void CRouteServConn::_HandleMsgReadNotify(CImPdu *pPdu)
{
    ...
    DEBUG_I("HandleMsgReadNotify, user_id=%u, session_id=%u, session_type=%u, msg_id=%u. ", req_id, session_id, session_type, msg_id);
...
void CRouteServConn::_HandleP2PMsg(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleP2PMsg, %u->%u ", from_user_id, to_user_id);
...
void CRouteServConn::_HandleUsersStatusResponse(CImPdu* pPdu)
{
	...
	DEBUG_I("HandleUsersStatusResp, user_id=%u, query_count=%u ", user_id, result_count);
    ...
    else if (attach_data.GetType() == ATTACH_TYPE_PDU_FOR_PUSH)
    {
        ...
        if (user_stat.status() == IM::BaseDefine::USER_STATUS_ONLINE)
        {
            ...
            DEBUG_I("HandleUsersStatusResponse, user id: %d, push type: normal. ", user_stat.user_id());
        }
        else
        {
...
void CRouteServConn::_HandleRemoveSessionNotify(CImPdu *pPdu)
{
    ...
    uint32_t user_id = msg.user_id();
    uint32_t session_id = msg.session_id();
    DEBUG_I("HandleRemoveSessionNotify, user_id=%u, session_id=%u ", user_id, session_id);
    CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(user_id);
    if (pUser)
    {
        pUser->BroadcastPdu(pPdu);
...
void CRouteServConn::_HandlePCLoginStatusNotify(CImPdu *pPdu)
{
    ...
    DEBUG_I("HandlePCLoginStatusNotify, user_id=%u, login_status=%u ", user_id, login_status);
...
void CRouteServConn::_HandleSignInfoChangedNotify(CImPdu* pPdu) {
        ...
        DEBUG_I("HandleSignInfoChangedNotify, changed_user_id=%u, sign_info=%s ", msg.changed_user_id(), msg.sign_info().c_str());
